...
namespace sn
{
	void CheckCompileErrors(uint32_t p_Shader, const std::string& p_Type)
	{
		...
		if (p_Type == "PROGRAM")
		{
			glGetProgramiv(p_Shader, GL_LINK_STATUS, &f_Compiled);
			if (f_Compiled == GL_FALSE)
			{
				glGetProgramiv(p_Shader, GL_INFO_LOG_LENGTH, &f_Size);
				std::vector<GLchar> f_ErrorLog(f_Size);
				glGetShaderInfoLog(p_Shader, f_Size, &f_Size, &f_ErrorLog[0]);
				SN_LOG_ERROR("ERROR::SHADER_COMPILATION_ERROR of type: {}", p_Type);
				SN_LOG_ERROR("{}", f_ErrorLog[0]);
			}
		}
		else
		{
			glGetShaderiv(p_Shader, GL_COMPILE_STATUS, &f_Compiled);
			if (f_Compiled == GL_FALSE)
			{
				glGetShaderiv(p_Shader, GL_INFO_LOG_LENGTH, &f_Size);
				std::vector<GLchar> f_ErrorLog(f_Size);
				glGetShaderInfoLog(p_Shader, f_Size, &f_Size, &f_ErrorLog[0]);
				SN_LOG_ERROR("ERROR::SHADER_COMPILATION_ERROR of type: {}", p_Type);
				SN_LOG_ERROR("{}", f_ErrorLog[0]);
	...
	OpenGLShader::OpenGLShader(const std::string& p_VertexPath, const std::string& p_FragmentPath)
	{
		...
		glShaderSource(f_Vertex, 1, &f_VertexSource, nullptr);
		glCompileShader(f_Vertex);
		CheckCompileErrors(f_Vertex, "VERTEX");

		// Fragment Shader
		uint32_t f_Fragment = glCreateShader(GL_FRAGMENT_SHADER);
		glShaderSource(f_Fragment, 1, &f_FragmentSource, nullptr);
		glCompileShader(f_Fragment);
		CheckCompileErrors(f_Fragment, "FRAGMENT");

		// Shader Program
		m_RendererID = glCreateProgram();
		glAttachShader(m_RendererID, f_Vertex);
		glAttachShader(m_RendererID, f_Fragment);
